  ...
  extractTop20 = function(line) {
    ...
    if (length(fields) == 21) {
     ...
     # Skip records where departure dalay is "NA":
      if (!(identical(desti, "NA"))&&identical(fields[[18]],"1")) {
        # field[9] is carrier, field[1] is year, field[2] is month:
        rhcollect(paste(fields[[1]],"\t",fields[[3]],"\t", desti, sep=""), 1)
      }
...
output <-"/results/NYC/Desti/Cancel-week"
z <- rhwatch(
  map=map,
  reduce=reduce,
  combiner=TRUE,
...
write(counts, file="NYCCancelDesti-week.csv")
