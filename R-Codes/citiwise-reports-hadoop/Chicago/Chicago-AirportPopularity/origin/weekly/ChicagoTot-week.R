    ...
    if (length(fields) == 21) {
     ...
     # Skip records where departure dalay is "NA":
      if (!(identical(origin, "NA"))) {
        ...
        rhcollect(paste(fields[[1]], "\t",fields[[3]], "\t",origin,sep=""), 1)
      }
...
output <-"/results/Chicago/origin/Traffic-week"
z <- rhwatch(
  map=map,
  reduce=reduce,
  combiner=TRUE,
  ...
  jobname='Week Airport in Chicago-Origin',
...
write(counts, file="ChicagoAirportOrigin-week.csv")
