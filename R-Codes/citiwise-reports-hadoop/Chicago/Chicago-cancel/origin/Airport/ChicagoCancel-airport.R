  ...
  extractTop20 = function(line) {
    ...
    if (length(fields) == 21) {
     ...
     # Skip records where departure dalay is "NA":
      if (!(identical(origin, "NA"))&&identical(fields[[18]],"1")) {
        # field[9] is carrier, field[1] is year, field[2] is month:
        rhcollect(paste(fields[[1]],"\t",fields[[2]],"\t",origin, sep=""), 1)
      }
...
input <-"/results/ChicagoOrigin/"
output <-"/results/Chicago/origin/Cancel-airport"
z <- rhwatch(
  map=map,
  reduce=reduce,
  combiner=TRUE,
  ...
  jobname='Chicago Cancel-airport',
...
write(counts, file="ChicagoCancel-airport.csv")
