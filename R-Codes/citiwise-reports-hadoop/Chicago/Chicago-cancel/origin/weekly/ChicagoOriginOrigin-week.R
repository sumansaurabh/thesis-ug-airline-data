    ...
    if (length(fields) == 21) {
     ...
     # Skip records where departure dalay is "NA":
      if (!(identical(origin, "NA"))&&identical(fields[[18]],"1")) {
        ...
        rhcollect(paste(fields[[1]], "\t",fields[[4]], "\t",origin,sep=""), 1)
      }
...
output <-"/results/Chicago/origin/Cancel-week"
z <- rhwatch(
  map=map,
  reduce=reduce,
  combiner=TRUE,
...
write(counts, file="ChicagoCancel-weekly.csv")
