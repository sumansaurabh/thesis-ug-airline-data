  ...
  extractTop20 = function(line) {
    ...
    if (length(fields) == 21) {
     ...
     # Skip records where departure dalay is "NA":
        ...
        if(identical(origin,"CGX")||identical(origin,"MDW")||identical(origin,"ORD"))
...
input <-"/results/ChicagoLosAngelesWashington/"
output <-"/results/ChicagoOrigin"
z <- rhwatch(
  map=map,
  reduce=reduce,
  ...
  jobname='Chicago Origin Details',
...
write(counts, file="Chicagoorigin.dat")
