...
library("ggplot2")
library(scales)
airport<-read.csv("/home/perfectus/Desktop/btp/airline-dataset/rhipe-airlie/resultset/citiwise-reports/Chicago/Chicago-cancel/origin/Airport/ChicagoCancel-airport.csv",sep="\t",header=F)
...
p1<-ggplot(port, aes(x=TimeLine, y=visits, colour=airport, group=iata)) +
  geom_line() +
  ggtitle("Cancelled Flights Traffic ")+
  xlab("Year")+ylab("Number of Cancelled Flights")+
