...
library("ggplot2")
library(scales)
airport<-read.csv("/home/perfectus/Desktop/btp/airline-dataset/rhipe-airlie/resultset/citiwise-reports/Chicago/Chicgo-delay/origin/Airport/ChicagoDelay-airport.csv",sep="\t",header=F)
...
p1<-ggplot(port, aes(x=TimeLine, y=visits, colour=airport, group=iata)) +
  geom_line() +
  ggtitle("Traffic of Flights Delayed(>15 min)")+
  xlab("Year")+ylab("Number of Flights Delayed")+
