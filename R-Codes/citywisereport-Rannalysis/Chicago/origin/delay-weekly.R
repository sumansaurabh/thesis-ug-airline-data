...
airport<-read.csv("/home/perfectus/Desktop/btp/airline-dataset/rhipe-airlie/resultset/citiwise-reports/Chicago/Chicgo-delay/origin/weekly/ChicagoDelay-week.csv",sep="\t",header = F)
...
p1<-ggplot(port[which(port$year=="2008")], aes(x=reorder(dayname,day), y=visits, colour=airport, group=iata)) +
  geom_line() +
  ggtitle("Flight Delayed (Year 2008) ")+
  xlab("")+ylab("Delayed Flights")
p1
...
grid.arrange(p1,p2,p3,p4,p5,ncol = 1, main = "New York Weekly Delayed Traffic")
grid.arrange(p6,p7,p8,p9,p10,ncol = 1, main = "New York Weekly Delayed Traffic")
