...
airport<-read.csv("/home/perfectus/Desktop/btp/airline-dataset/rhipe-airlie/resultset/citiwise-reports/NYC/NYC-DelayRate/Origin/weekly/NYCDelay-week.csv",sep="\t",header = F)
...
p1<-ggplot(port[which(port$year=="2008")], aes(x=reorder(dayname,day), y=visits, colour=airport, group=iata)) +
  geom_line() +
  ggtitle("Delay Weekly (Year 2008) ")+
  xlab("")+ylab("Delayed Traffic")
p1
