library(data.table)
library(googleVis)
airport<-read.csv("/home/perfectus/Desktop/btp/airline-dataset/rhipe-airlie/resultset/citiwise-reports/NYC/NYC-AirportPopularity/origin/NYCAirportOriginMath.csv",sep="\t")
airport<-data.table(airport)
setnames(airport,"X1988","year")
setnames(airport,"LGA","iata")
setnames(airport,"X1","month")
setnames(airport,"X9685.0","visits")
...
#qplot(year,visits,data=data2,geom="line",linetype=variable)
